...
import { GameState, ShotDirection, Ball, Batsman, Bat, Stumps, GameContextForCommentary, AssetsLoaded, TutorialStep, DeliveryType } from '../types';
import {
    ...
    PITCH_BOUNCE_Y_POSITION, SWING_MAGNITUDE, CUTTER_MAGNITUDE
} from '../constants';
...
export function useGameEngine({ assets, commentary }: UseGameEngineProps) {
    ...
    const initGameElements = useCallback(() => {
        ...
        setBall({ x: CANVAS_WIDTH / 2, y: CANVAS_HEIGHT - 38, z: 0, radius: 8, dx: 0, dy: 0, dz: 0, trail: [], deliveryType: null, swingFactor: 0, cutterFactor: 0, hasBounced: false });
    ...
    const bowlLogic = useCallback(() => {
        ...
        const selectDelivery = (): DeliveryType => {
            const probabilityPool: DeliveryType[] = ['FAST', 'FAST', 'SLOWER', 'INSWINGER', 'OUTSWINGER', 'LEG_CUTTER', 'OFF_CUTTER'];
            return probabilityPool[Math.floor(Math.random() * probabilityPool.length)];
        };
        
        const deliveryType = selectDelivery();
        ...
        let swing = 0;
        let cutter = 0;
        let startX = CANVAS_WIDTH / 2 + (Math.random() - 0.5) * 20;

        switch(deliveryType) {
            case 'FAST':
                speed = MAX_BALL_SPEED_Y - Math.random() * 1.5;
                break;
            case 'SLOWER':
                speed = MIN_BALL_SPEED_Y + Math.random() * 1.5;
                break;
            case 'INSWINGER': // Swings towards the leg side for a right-hander
                speed = (MAX_BALL_SPEED_Y + MIN_BALL_SPEED_Y) / 2 + (Math.random() - 0.5) * 2;
                swing = SWING_MAGNITUDE;
                startX -= 20; // Start wider from off
                break;
            case 'OUTSWINGER': // Swings away to the off side
                speed = (MAX_BALL_SPEED_Y + MIN_BALL_SPEED_Y) / 2 + (Math.random() - 0.5) * 2;
                swing = -SWING_MAGNITUDE;
                startX += 20; // Start wider from leg
                break;
            case 'LEG_CUTTER': // Cuts away to off side after bouncing
                speed = (MAX_BALL_SPEED_Y + MIN_BALL_SPEED_Y) / 2 + (Math.random() - 0.5) * 2;
                cutter = -CUTTER_MAGNITUDE;
                break;
            case 'OFF_CUTTER': // Cuts in to leg side after bouncing
                speed = (MAX_BALL_SPEED_Y + MIN_BALL_SPEED_Y) / 2 + (Math.random() - 0.5) * 2;
                cutter = CUTTER_MAGNITUDE;
                break;
        }
        ...
        setBall(prev => prev ? { 
            ...
            z: 0, 
            dz: 0, 
            dx: (CANVAS_WIDTH / 2 - startX) / 80, // slight initial angle towards stumps
            dy: -speed, 
            trail: [], 
        ...
        showAppMessage(`${deliveryType.replace('_', ' ')}!`, 1500);

        commentary.triggerDynamicCommentary({
            event: "bowlerApproaching",
            deliveryType: deliveryType,
    ...
    }, [showAppMessage, stateRefs, commentary, totalBalls]);
    ...
    const updateBallPosition = useCallback(() => {
        ...
        if (cState === 'BOWLING' || cState === 'HITTING') {
            ...
            // AI Bowling Physics (only for non-tutorial balls)
            if (!isTutorialSwing) {
                // Apply swing before bounce
                if (!newBall.hasBounced) {
                    newBall.dx += newBall.swingFactor;
                }
                
                // Bounce logic
                if (!newBall.hasBounced && newBall.y <= PITCH_BOUNCE_Y_POSITION) {
                    newBall.hasBounced = true;
                    // Apply cutter effect on bounce
                    if (newBall.cutterFactor !== 0) {
                        newBall.dx += newBall.cutterFactor;
                        newBall.dy *= 0.9; // Slow the ball a bit on cut
                    }
                    // Make it bounce up
                    newBall.dz = 3 + Math.random(); 
                }

                // Vertical movement after bounce
                if (newBall.hasBounced) {
                    newBall.dz -= GRAVITY / 3; 
                    newBall.z += newBall.dz;
                    if (newBall.z < 0) {
                        newBall.z = 0;
                        newBall.dz = 0;
                    }
